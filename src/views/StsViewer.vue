...
<script setup lang="ts">
  ...
  const processInput = () => {
    ...
    outputText.value = convertAsciiHexToText(textResult);
  }

  const removeUnwantedIndex = (input: string[]) => {
  ...
  const buildStringFromArray = (input: string[]) => {
    let strOutput = "";
  ...
  const convertAsciiHexToText = (input: string) => {
    input = input.replace(/\\x([0-9a-f]{2})/ig, function (_, pair) {
